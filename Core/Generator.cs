...
public static class Generator
{
    ...
    internal static void ApplyWatermark(string modDir, MapConfig cfg)
    {
        ...
        string version = (typeof(Generator).Assembly
        ...
        // DirectoryInfo rather than Directory: the FileInfo it yields already carries the length
        // from the directory entry, so the "blanked vanilla override" test below costs no stat of
        // its own. Three quarters of the files in a written mod are those one-byte blanks, and this
        // is what keeps them from being opened at all.
        var candidates = new DirectoryInfo(modDir)
        ...
        // realtime scanner reading each file behind us. Each file is independent; no iteration
        // reads a path another one writes.
        Parallel.ForEach(candidates, file =>
        {
            ...
            if (content.Contains("Generated by Ck3MapGen")) return;

            // Strip existing UTF-8 BOM if present so ParadoxText.WriteBom doesn't duplicate it
            if (content.Length > 0 && content[0] == '\uFEFF')
                content = content[1..];

            Io.ParadoxText.WriteBom(file.FullName, watermark + content);
        });
    }
