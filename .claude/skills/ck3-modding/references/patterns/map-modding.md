...
## Verified on this install (CK3 1.19) — read before the rest of this file
...
| File | Dimensions | PNG format |
|---|---|---|
| `provinces.png` | **9216 x 4608** | 8 bit, colour type 2 (truecolour RGB, no alpha) |
| `rivers.png` | **9216 x 4608** | 8 bit, colour type 3 (**indexed palette**) |
| `heightmap.png` | **18432 x 9216** — exactly 2x provinces in each axis | **16 bit**, colour type 0 (grayscale) |
| `packed_heightmap.png` | 3185 x 4061 (an atlas; not a clean tile grid — vanilla packs 5 mip levels via `level_offsets`) | 16 bit, colour type 0 |
| `indirection_heightmap.png` | 288 x 144 (= tile grid of the 18432-wide heightmap at `tile_size` 65) | 8 bit, colour type 6 (RGBA) |

Vanilla `heightmap.heightmap` in full — it is a short text index, and the numbers in it must
agree with the two PNGs above:
...
| Fact | Correct value | Wrong in the text below |
|---|---|---|
| `heightmap.png` bit depth | **16-bit grayscale** (`PngWriter.WriteGray16`) | claims 8-bit grayscale |
| Sea level | **19 on the 0–255 scale** = `19 * 257` = 4883 in 16-bit. From `WATERLEVEL = 3 ### 0.06 in 0-1, 19 in 0-255` in `<game>\common\defines\00_defines.txt` | claims ~95 |
| One 0–255 step in 16-bit | 257 raw units exactly (`255 * 257 == 65535`) | not mentioned |
| `packed_heightmap.png` + `indirection_heightmap.png` | **Required build products, not caches.** Either ship a correct pair or ship a bare `heightmap.png` and repack in `-mapeditor`. `heightmap.heightmap` carries the matching `level_offsets` and must be regenerated with them | says "generated/cached, do not edit" |
| Indirection pixel layout | RGBA8: **R = atlas column, G = atlas row, B = 2^level, A = detail level** | not documented |
| `rivers.png` background | **White (index 255) is LAND; water is index 254 (magenta).** Vanilla is ~23.4M white land px over ~18.8M magenta water px. Filling the oceans white asserts the sea is dry land | says background "white or transparent" |
| `rivers.png` colour matching | CK3 keys off **palette indices**, not RGB. Index 0 source, 1 join, 2 split, 3–11 narrowest→widest flow. The palette must be reproduced exactly | implies RGB values are what matter |
| `provinces.png` | 8 bits **per channel** RGB (i.e. 24-bit), no alpha | "8-bit RGB" is ambiguous; 8-bit *indexed* would be wrong |

**Two silent ceilings on map size** (both crash or corrupt with nothing logged):

1. The indirection stores atlas coordinates in single **bytes** — no atlas axis may exceed 256
   tiles. Laying tiles out 1:1 with the source grid breaks at vanilla's own size (288 tiles wide,
   R wraps to 32, silently pointing a third of the map at the wrong terrain). Pack tiles in
   reading order into an atlas at most 256 columns wide instead.
2. D3D11 caps a `Texture2D` at **16384 px per side**, and the packed heightmap is a texture. At
   `tile_size 65` that is 252 columns, not 256. Exceeding it returns a null texture and CK3
   crashes reading the texture description at +0x28 on a worker thread, unlogged.

**BOM rule for map files.** It is not uniform, and SKILL.md golden rule 4 only covers
localization. First three bytes of each vanilla file, checked 2026-08-10:

- **No BOM** — `definition.csv`, `default.map`, `adjacencies.csv`, `seasons.txt`,
  `island_region.txt`, `climate.txt`, `positions.txt`.
- **UTF-8 BOM (`ef bb bf`)** — `heightmap.heightmap`, plus script files under `common/`,
  `history/`, `gfx/` and `map_data/geographical_regions/`.
- Emit LF, not CRLF.
...
- Standard vanilla map size: **9216 x 4608** pixels (measured; the "8192 x 4096" figure repeated throughout this imported file is wrong for 1.19)

### heightmap.png -- Elevation Data
Grayscale image defining terrain elevation. Darker = lower, brighter = higher.

**Requirements** (corrected — the imported values were wrong; see the verified box at the top):
- Format: **16-bit** grayscale PNG
- **Twice** provinces.png's resolution in each axis (vanilla: 18432 x 9216 heightmap over a 9216 x 4608 province map), so four heightmap pixels cover one province pixel
- Sea level is **19 on the 0-255 scale** (`19 * 257` = 4883 in 16-bit), from `WATERLEVEL` in `00_defines.txt` — not ~95
- `heightmap.heightmap` is a small text index (it carries `level_offsets` and `tile_size`), not something the game derives for you at load
- `packed_heightmap.png` and `indirection_heightmap.png` are **required build products**. Either generate a correct, consistent trio (packed + indirection + heightmap.heightmap) or ship a bare `heightmap.png` and repack in `-mapeditor`. Shipping a stale pair against a new heightmap is the failure mode
...
- **Wrong:** background is NOT "white or transparent". White (palette index 255) is **land**; sea, ocean and lake pixels are **index 254 (magenta)**. Between them they cover the whole map. Painting the oceans white asserts the sea is dry land
- CK3 matches on **palette index**, not RGB — reproduce vanilla's palette exactly (index 0 source, 1 join, 2 split, 3-11 narrowest to widest)
...
|------|--------|------------|---------------------|
| provinces.png | PNG | 8 bits/channel RGB, no alpha (IHDR colour type 2) | **9216 x 4608** |
| heightmap.png | PNG | **16-bit** Grayscale (colour type 0) | **18432 x 9216** (2x provinces) |
| rivers.png | PNG | 8-bit Indexed Colour (colour type 3) | **9216 x 4608** |
| packed_heightmap.png | PNG | 16-bit Grayscale | atlas; vanilla 3185 x 4061 |
| indirection_heightmap.png | PNG | 8-bit RGBA (colour type 6) | tile grid; vanilla 288 x 144 |
...
- **Heightmap sea level**: If your heightmap values for water areas are too high, they render as land visually even though the game treats them as sea. Keep sea areas at or below the threshold, which is **19 on the 0-255 scale** (4883 in the 16-bit file) -- *not* ~95. Conversely, land painted below it renders as sea. Because the province map is half the heightmap's resolution, the two coastlines only agree if each heightmap pixel is clamped to the side of sea level its province is on
